function getViewApprovedPost(){
  $.ajax({url: "../../func/viewApprovedPost.php",    
    beforeSend: function() {
    ...
    if(data.result == 1 && data.role == "admin"){
        ...
        $(data.data).each(function (index, post) {
            ...
            html += post.title;
            html += '</a>';
            html += '<p class="mb-1 text-white text-small w-15 w-xs-100">' + post.category + '</p>';
            html += '<p class="mb-1 text-white text-small w-15 w-xs-100">' + post.date + '</p>';
            ...
            html += '<button type="button" class="btn btn-outline-danger mb-2" onclick="archivePost('+post.id+');">Archive</button>';
...
function getViewDisapprovedPost(){
  $.ajax({url: "../../func/viewDisapprovedPost.php",    
    beforeSend: function() {
    ...
    if(data.result == 1 && data.role == "admin"){
        ...
        $(data.data).each(function (index, post) {
            ...
            html += '</div>';
            html += '<div class="custom-control mb-0 align-self-center mr-4" style="display:inline-grid;">';
            html += '<button type="button" class="btn btn-outline-success mt-2 mb-2" onclick="approvePost('+post.id+');">Approve</button>';
            html += '<button type="button" class="btn btn-outline-danger mb-2" onclick="disapprovePost('+post.id+');">Decline</button>';
            html += '</div>';
            html += '</div>';
            html += '</div>';
        });
...
function getViewArchivePost(){
  $.ajax({url: "../../func/viewArchivePost.php",    
    beforeSend: function() {
        $(".archivePost").html('<div class="loader"></div>');
    }, success: function(response){
    var data = $.parseJSON(response)
    console.log(data);
    if(data.result == 1 && data.role == "admin"){
        var html = "";
        if(data.data.length == 0)
        html +='<span class="badge badge-pill badge-danger">No Archive Post</span>';             
        else{
        $(data.data).each(function (index, post) {
            var escapeTitle = escape(post.title);
            var escapeContent = escape(post.content); 
            ...
            html += '<p class="mb-1 text-white text-small w-15 w-xs-100">' + post.date + '</p>';
            html += '<div class="w-15 w-xs-100">'
            html += '<span class="badge badge-pill badge-danger">Archive</span>';
            html += '</div>';
            html += '</div>';
            html += '<div class="custom-control mb-0 align-self-center mr-4" style="display:inline-grid;">';
            html += '<button type="button" class="btn btn-outline-danger mb-2" onclick="unarchivePost('+post.id+');">Unarchive</button>';
            html += '</div>';
            html += '</div>';
            html += '</div>';
        });
